  ...
  methods: {
    getSmsCode() {
      ...
      this.$store.dispatch('GetSmsCode', this.loginForm.username).then(() => {
        ...
        this.smsCodeTotalTime--
        this.smsCodeContent = this.smsCodeTotalTime + 's后重新发送'
        // 倒计时
        const clock = window.setInterval(() => {
          if (this.smsCodeTotalTime <= 0) {
            window.clearInterval(clock)
            this.smsCodeContent = '重新获取验证码'
            this.smsCodeTotalTime = 60
    ...
    handleLogin() {
      this.$refs.loginForm.validate(valid => {
        if (valid) {
          this.loading = true
          this.$store.dispatch('VerifyMobile', this.loginForm).then(() => {
            this.loading = false
